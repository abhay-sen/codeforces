...
void finding(string& str,char key,int m,int &count){
    int n=str.length();
    int i=0;
    while(i<n&&m!=0){
        if(str[i]==key) m--;
...
void solve()
{
    ...
    string str;cin>>str;
    int count=0;
    finding(str,'A',m,count);
    finding(str, 'B', m, count);
    finding(str, 'C', m, count);

    finding(str, 'D', m, count);
    finding(str, 'E', m, count);
    finding(str, 'F', m, count);
    finding(str, 'G', m, count);
    cout<<count<<endl;
}
